...
export const changeUserRoleSchema = z
  .object({
    ...
    reason: z
      .string()
      .min(1, '역할 변경 사유는 필수입니다')
      .max(500, '사유는 500자를 초과할 수 없습니다'),
  })
  .strict();
...
export const bulkUserActionSchema = z
  .object({
    userIds: z
      ...
      .max(100, '한 번에 최대 100명까지 처리할 수 있습니다'),
    action: z.enum(['activate', 'deactivate', 'verify', 'delete'], {
      errorMap: () => ({
        message:
          '액션은 activate, deactivate, verify, delete 중 하나여야 합니다',
      }),
    }),
    reason: z
      .string()
...
export function validateUserQuery(query: any): UserSearchQuery {
  ...
  if (!result.success) {
    throw new Error(
      `잘못된 사용자 검색 쿼리: ${result.error.errors.map((e) => e.message).join(', ')}`
    );
  }
  return result.data;
}
